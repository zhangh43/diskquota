...
add_db_to_config(Oid dbid)
{
	...
	elog(LOG,"insert diskquota_namespace :%s",str.data);
 ...
 * if we 
 * LWLockAcquire(diskquota_locks.message_box_lock, LW_EXCLUSIVE);know the exact error which caused failure,
...
disk_quota_launcher_main(Datum main_arg)
{
	...
	while (!got_sigterm)
	{
		...
		{
			got_sigusr1 = false;
			elog(LOG,"launcher begin process message");
			process_message_box();
		}
		...
		switch (local_message_box.cmd)
		{
			...
			case CMD_DROP_EXTENSION:
				...
				*code = ERR_OK;
				break;
			default:
				elog(LOG, "[diskquota]:unsupported message cmd=%d", local_message_box.cmd);
				*code = ERR_UNKNOWN;
				break;
		}
	}
